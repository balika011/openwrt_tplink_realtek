...
static int32
_phy_rtl826xb_patch_wait(struct phy_device *phydev, uint32 mmdAddr, uint32 mmdReg, uint32 data, uint32 mask)
{
    int32  ret = 0;
    uint32 rData = 0;
    uint32 cnt = 0;
...
static int32
_phy_rtl826xb_patch_wait_not_equal(struct phy_device *phydev, uint32 mmdAddr, uint32 mmdReg, uint32 data, uint32 mask)
{
    int32  ret = 0;
    uint32 rData = 0;
    uint32 cnt = 0;
...
static int32
_phy_rtl826xb_patch_top_get(struct phy_device *phydev, uint32 topPage, uint32 topReg, uint32 *pData)
{
    int32  ret = 0;
    uint32 rData = 0;
    uint32 topAddr = (topPage * 8) + (topReg - 16);

    if ((ret = phy_common_general_reg_mmd_get(phydev, PHY_MMD_VEND1, topAddr, &rData)) != RT_ERR_OK)
        return ret;
    *pData = rData;
    return RT_ERR_OK;
}
...
static int32
_phy_rtl826xb_patch_top_set(struct phy_device *phydev, uint32 topPage, uint32 topReg, uint32 wData)
{
    int32  ret = 0;
    uint32 topAddr = (topPage * 8) + (topReg - 16);
    if ((ret = phy_common_general_reg_mmd_set(phydev, PHY_MMD_VEND1, topAddr, wData)) != RT_ERR_OK)
        return ret;
    return RT_ERR_OK;
}

static int32
_phy_rtl826xb_patch_sds_get(struct phy_device *phydev, uint32 sdsPage, uint32 sdsReg, uint32 *pData)
{
    int32  ret = 0;
    uint32 rData = 0;
    ...
    if ((ret = _phy_rtl826xb_patch_top_get(phydev, 40, 18, &rData)) != RT_ERR_OK)
        return ret;
    *pData = rData;
    return _phy_rtl826xb_patch_wait(phydev, PHY_MMD_VEND1, 0x143, 0, BIT_15);
}

static int32
_phy_rtl826xb_patch_sds_set(struct phy_device *phydev, uint32 sdsPage, uint32 sdsReg, uint32 wData)
{
    int32  ret = 0;
...
static int32 _phy_rtl826xb_flow_r1(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    ...
    RT_ERR_CHK(_phy_rtl826xb_patch_wait(phydev, 31, _phy_rtl826xb_mmd_convert(0xb80, 16), BIT_6, BIT_6), ret);
...
static int32 _phy_rtl826xb_flow_r12(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_r2(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
    ...
    //set patch_rdy [PHYReg_bit r $PHYID 0xb80 16 6 6] ; Wait for patch ready != 1
    RT_ERR_CHK( _phy_rtl826xb_patch_wait_not_equal(phydev, 31, _phy_rtl826xb_mmd_convert(0xb80, 16), BIT_6, BIT_6), ret);

    return RT_ERR_OK;
}

static int32 _phy_rtl826xb_flow_l1(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
    ...
    RT_ERR_CHK( _phy_rtl826xb_patch_wait(phydev, 31, _phy_rtl826xb_mmd_convert(0xa60, 16), 0x1, 0xFF), ret);

    return RT_ERR_OK;
}

static int32 _phy_rtl826xb_flow_l2(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
    ...
    //set pcs_state [PHYReg_bit r $PHYID 0xa60 16 7 0] ; Wait for pcs state != 1
    RT_ERR_CHK( _phy_rtl826xb_patch_wait_not_equal(phydev, 31, _phy_rtl826xb_mmd_convert(0xa60, 16), 0x1, 0xFF), ret);

     return RT_ERR_OK;
}

static int32 _phy_rtl826xb_flow_pi(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
    ...
    if ((ret = phy_common_general_reg_mmd_get(phydev, PHY_MMD_VEND2, 0xbc62, &rData)) != RT_ERR_OK)
        return ret;
...
static int32 _phy_rtl826xb_flow_n01(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_n02(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_n11(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_n12(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_n21(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_n22(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_s(struct phy_device *phydev)
{
...
static int32 _phy_rtl826xb_flow_cmpstart(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 _phy_rtl826xb_flow_cmpend(struct phy_device *phydev)
{
    int32 ret = RT_ERR_OK;
    rt_phy_patch_db_t *pPatchDb = NULL;
...
static int32 phy_rtl826xb_patch_op(struct phy_device *phydev, rtk_hwpatch_t *pPatch_data)
{
    int32 ret = RT_ERR_OK;
    uint32 rData = 0, wData = 0;
    uint16 reg = 0;
    uint32 mask = UINT32_BITS_MASK(pPatch_data->msb, pPatch_data->lsb);
    ...
    switch (pPatch_data->patch_op)
    {
        case RTK_PATCH_OP_PHY:
            ...
            {
                RT_ERR_CHK(phy_common_general_reg_mmd_get(phydev, 31, reg, &rData), ret);
            }
            wData = REG32_FIELD_SET(rData, pPatch_data->data, pPatch_data->lsb, mask);
            RT_ERR_CHK(phy_common_general_reg_mmd_set(phydev, 31, reg, wData), ret);
            break;

        case RTK_PATCH_OP_PHYOCP:
            if ((pPatch_data->msb != 15) || (pPatch_data->lsb != 0))
            {
                RT_ERR_CHK(phy_common_general_reg_mmd_get(phydev, 31, pPatch_data->addr, &rData), ret);
            }
            wData = REG32_FIELD_SET(rData, pPatch_data->data, pPatch_data->lsb, mask);
            RT_ERR_CHK(phy_common_general_reg_mmd_set(phydev, 31, pPatch_data->addr, wData), ret);
            break;
...
static int32 phy_rtl826xb_patch_flow(struct phy_device *phydev, uint8 patch_flow)
{
    int32 ret = RT_ERR_OK;
...
int32 phy_rtl826xb_patch_db_init(struct phy_device *phydev, rt_phy_patch_db_t **pPhy_patchDb)
{
    int32 ret = RT_ERR_OK;
